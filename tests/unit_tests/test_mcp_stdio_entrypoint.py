...
def _install_entrypoint_stubs(monkeypatch: pytest.MonkeyPatch) -> None:
    superset_module = ModuleType("superset")
    superset_module.__path__ = []  # type: ignore[attr-defined]

    mcp_service_module = ModuleType("superset.mcp_service")
    mcp_service_module.__path__ = []  # type: ignore[attr-defined]

    app_module = ModuleType("superset.mcp_service.app")
    app_module.init_fastmcp_server = MagicMock()
    app_module.mcp = MagicMock()

    middleware_module = ModuleType("superset.mcp_service.middleware")
    middleware_module.create_response_size_guard_middleware = lambda: None

    server_module = ModuleType("superset.mcp_service.server")
