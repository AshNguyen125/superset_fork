...
BEGIN
  INSERT INTO public.users (name, real_name)
    VALUES ('test_name', 'test_real_name');
END;
$$;
            """,
            True,
...
def test_is_mutating_nested_dml() -> None:
    ...
    sql = """
    WITH updated AS (
      UPDATE orders SET col = 1 WHERE id = 1 RETURNING *
    )
    SELECT * FROM updated;
    """
    assert SQLStatement(sql, "postgresql").is_mutating() is True
...
def test_extract_tables_insert_with_column_list() -> None:
    ...
    """
    tables = SQLStatement(
        "INSERT INTO orders (id, col) VALUES (1, 2)", "postgresql"
    ).tables
    assert Table("orders") in tables
...
def test_strip_jinja() -> None:
    ...
    # Block tags without SQL keywords are removed entirely (empty string).
    assert strip_jinja("{% set x = 1 %} x {% endif %}") == " x "
    assert strip_jinja("{% if is_admin() %} x {% endif %}") == " x "
    # Block tags WITH SQL control keywords are replaced with the synthetic
    # subquery marker so has_subquery() remains fail-closed.
    assert strip_jinja("{% if SELECT 1 %} x {% endif %}") == "(SELECT 1) x "
    assert strip_jinja("{% DELETE FROM users %}") == "(SELECT 1)"
...
def test_is_mutating_copy() -> None:
    """
    Test that COPY (both as exp.Copy AST node and as exp.Command) is mutating.
    """
    from superset.sql.parse import SQLStatement

    # Standard COPY — sqlglot parses this as exp.Copy
    assert SQLStatement("COPY foo FROM 'file.csv'", "postgresql").is_mutating() is True
    assert (
        SQLStatement("COPY foo TO '/tmp/out.csv'", "postgresql").is_mutating() is True
    )


def test_extract_tables_from_statement_merge() -> None:
    """
    MERGE INTO target USING source — the target table must appear in the
    extracted table set so that RLS predicates are applied to it.
    """
    from superset.sql.parse import extract_tables_from_statement

    sql = (
        "MERGE INTO protected_table "
        "USING src ON protected_table.id = src.id "
        "WHEN MATCHED THEN UPDATE SET col = src.col"
    )
    statement = parse_one(sql, dialect="spark")
    tables = extract_tables_from_statement(statement, Dialects.SPARK)
    table_names = {t.table for t in tables}
    assert "protected_table" in table_names, (
        "MERGE target table must be extracted so RLS checks are applied to it"
    )
